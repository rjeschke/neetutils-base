 ...
 */
public class ShortList implements RandomAccess, Cloneable, Serializable
{
    /** serialVersionUID */
    private static final long serialVersionUID = -1363574360263955286L;
    /** Initial size. */
     ...
     * @return The list.
     */
    public static ShortList fromArray(final short... values)
    {
        final ShortList list = new ShortList(values.length);
     ...
     * @return The added element.
     */
    public short add(final int index, final short v)
    {
        if(index == this.size)
            return this.add(v);
     ...
     */
    public short remove(final short index)
    {
        if(index >= this.size)
            throw new ArrayIndexOutOfBoundsException(index);
        final short old = this.data[index];
        this.size--;
        if(index != this.size)
        {
     ...
     */
    public short set(final int index, final short v)
    {
        if(index >= this.size)
            throw new ArrayIndexOutOfBoundsException(index);
     ...
     */
    public int indexOf(final short v)
    {
     ...
     * @return The index or <code>-1</code> if the value was not found.
     */
    public int lastIndexOf(final short v)
    {
    ...
    public boolean equals(final Object other)
    {
        ...
        final ShortList list = (ShortList)other;
    ...
    {
        final ShortList list = new ShortList(this.size);
        list.size = this.size;
