...
public class DoubleList implements RandomAccess, Cloneable, Serializable
{
     ...
     * @return The list.
     */
    public static DoubleList fromArray(final double... values)
    {
        final DoubleList list = new DoubleList(values.length);
     ...
     * @return The added element.
     */
    public double add(final int index, final double v)
    {
    ...
    public double remove(final int index)
    {
        ...
        final double old = this.data[index];
        this.size--;
        if (index != this.size)
        {
     ...
     */
    public double set(final int index, final double v)
    {
        if (index >= this.size) throw new ArrayIndexOutOfBoundsException(index);
     ...
     */
    public int indexOf(final double v)
    {
     ...
     * @return The index or <code>-1</code> if the value was not found.
     */
    public int lastIndexOf(final double v)
    {
    ...
    public int hashCode()
    {
        ...
        for (int i = 0; i < this.size; i++)
        {
            final long h = Double.doubleToLongBits(this.data[i]);
    ...
    public boolean equals(final Object other)
    {
        ...
        final DoubleList list = (DoubleList)other;
    ...
    {
        final DoubleList list = new DoubleList(this.size);
        list.size = this.size;
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException
    {
        ...
        this.data = new double[this.maxSize = this.size];
        for (int i = 0; i < this.size; i++)
        {
