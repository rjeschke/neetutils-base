...
public class NImage implements WorkerCallback<NImagePBlock>
{
    ...
    public void drawLine(int x0, int y0, int x1, int y1, List<NPoint> points)
    {
        int x, y;
        int dx, dy, xs, ys, a;
    ...
    public void fillPolygon(NPoint[] points, NColor color)
    {
        ...
        for(int i = 0; i < points.length - 1; i++)
            this.drawLine(points[i].x, points[i].y, points[i + 1].x, points[i + 1].y, dpoints);

        if(!points[0].equals(points[points.length - 1]))
            this.drawLine(points[points.length - 1].x, points[points.length - 1].y, points[0].x, points[0].y, dpoints);
    ...
    public void fillPolygon(List<NPoint> points, NColor color)
    {
        ...
        for(int i = 0; i < points.size() - 1; i++)
            this.drawLine(points.get(i).x, points.get(i).y, points.get(i + 1).x, points.get(i + 1).y, dpoints);

        if(!Colls.head(points).equals(Colls.last(points)))
            this.drawLine(Colls.last(points).x, Colls.last(points).y, Colls.head(points).x, Colls.head(points).y,
                    dpoints);
