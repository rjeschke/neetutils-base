...
public final class ResamplerFactory
{
            ...
            final double attenuation)
    {
        ...
        if (srcFs < targetFs)
        {
            ...
            return new ResamplerUp(srcFs, targetFs, FIRUtils.multiply(
                    FIRUtils.createLowpass(win.length - 1, srcFs * 0.5 - tw, targetFs), win));
        }

        final double tw = targetFs * 0.5 * safeTw;
        win = FIRUtils.windowKaiser(tw, attenuation, srcFs);
        return new ResamplerDown(srcFs, targetFs, FIRUtils.multiply(
                FIRUtils.createLowpass(win.length - 1, targetFs * 0.5 - tw, srcFs), win));
    }
}
